    ...
    public class PageBar
    {
        public static string GetpageBar(int pageIndex, int pageCount)
        {
            ...
            if (pageIndex > 1)
            {
                ...
                //新版本的ajax页面中我们不需要再跳转页面了
                sb.AppendFormat("<a class='flip' pageIndex='{0}'>上一页</a>", pageIndex - 1);
            }
            for (int i = start; i <= end; i++)
            {
                ...
                else
                {
                    ...
                    sb.AppendFormat("<a  class='flip' pageIndex='{0}'>{0}</a>", i);
                }
            }
            if (pageIndex < pageCount)
            {
                //sb.AppendFormat("<a href='UserInfoAsp.aspx?pageIndex={0}'>下一页</a>", pageIndex + 1);
                sb.AppendFormat("<a  class='flip' pageIndex='{0}'>下一页</a>", pageIndex + 1);
